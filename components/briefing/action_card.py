...
from lib.levels import trade_levels
...
def _entry_target_invalidation_html(d: dict, ccy: str) -> str:
    ...
    The report-field mapping lives in ``lib.levels.trade_levels`` — shared with
    the Watchlist drill-down's four-cell levels plate, which draws the same
    device one size up and adds R:R. This function owns only the action card's
    own chrome. Returns "" when the report carries none of the three.
    """
    cells = trade_levels(d, ccy)
    if not cells:
        return ""
    return (
        ...
        + "".join(
            ...
            f'<div class="ac-tri-label">{c.label}</div>'
            f'<div class="ac-tri-val" style="color:{c.color};">{c.value}</div>'
            f'<div class="ac-tri-sub">{c.sub}</div>'
            f'</div>'
            for c in cells
        )
        + '</div>'
